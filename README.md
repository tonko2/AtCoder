...
D: 解けなかった. 高さを二分探索まではよかったが、その高さを使って何秒以内にできるかというリストを作って貪欲に判定する部分が思いつかなかった.
...
C: 解けなかった. 二分探索する起点をBではなくAだと思ってしまった.
...
113
A: 解けなかった. AB <= Kとなるような(A, B)の組はO(KlogK)個しかない(調和級数) ABC <= KもO(Klog^2K)で十分高速.
...
C: 解けなかった. 0の個数分右シフトして、1プラスして0の個数分左シフト. 最下位ビットが1のときは0の桁が変わってしまうので、左シフトする前にさらにプラスする.  難しい.
...
C: 解けなかった. 少数誤差. 5 3.45とかをA, B.Cにして、A * (100 * B + C)を計算すると1725. それに対して100割る.
...
C: 解けなかった. 偶奇で判定と、3手以内で全部いけるのはわかったが、判定方法がわからなかった. マンハッタン距離苦手.
...
C: 解けなかった. N = 10^4のとき、O(N^2)が大丈夫っていうのを忘れていた
...
C: 解けなかった. 2^(N - 1)じゃなくて2^Nだと思ってしまった.

198

B: 同じ文字列のrepeatは "0" * N
C: 解けなかった. R未満の距離いくのにくねくねして2回いくケースを見逃していた.
...
245
F: 解けなかった. 出字数が0の頂点は除外できる=辺を逆にした入次数0の頂点がきたらans--. トポロジカルソート.

254

D: 解けなかった. 平方数の性質を知っていないと解けない. 奇数を足していくと、平方数になる. 素因数分解して、余った数を掛けたら最小の平方数になる. 例えば8は2 * 2 * 2で、2を掛けたら最小の平方数になる. 8 * 2 * (平方数) -> (8 * (2 * (1 * 1))), (8 * (2 * (2 * 2)), (8 * (2 * (3 * 3))), ... をカウントしていけばいい.
E: 解けなかった 問題文をちゃんと読めば、愚直にできることがわかる. DFSとBFSの挙動を正確に把握してなかった. (1, 2), (2, 3), (3, 4), (1, 4), (4, 5) の辺集合の場合、1 -> 4 -> 5が2STEPでいけるが、 DFSで行うと最初に1 -> 2 -> 3 -> 4というSTEPで見る. だからsetで管理できない.

256

C: 解けた. 左上の4箇所が求まれば全部求まるから全探索
D: 解けた. ソートしてやるのもいいが、imos法で解ける.
E: 解けなかった. 出次数が1の有向グラフをFunctional Graphと呼び, Gの連結成分がK個あってC1, C2,...Ckとする. このとき各連結成分には閉路が1つだけ存在する. 重みを最小順にして、繋げていって、すでに登録済みの頂点がきたらそれは閉路になるので、その値を取り除く(答えに加算).
...
A: 解けなかった. 式変形まではよかった. 計算量の見積もりができてればループして終わり.
...
A: 解けなかった. L <= A, B, C <= Rを, 0 <= a, b, c <= R - Lに変形して、a = A - L, b = B - L, c = C - L. A - B = Cに代入して、a - L = b + c. a - L = xとおくて、0 + x ~ x + 0の範囲でaが存在する(x + 1)個.
a - L + 1のaに, LからR-Lの範囲で,代入 = 末項R-2L+1までの等差数列の和と等しい. 
...
A: 解けなかった. 値が大きくなっても, 素数の間隔は狭い. [1, 10]が与えられた時に, (1, 10), (1, 9), (2, 10), (1, 8), (2, 9), (3, 10)のように, 差を最大化しながらループしていく方法がわからなかった.
...
B: 解けたけど, もっと計算時間落とせる. K = min(K, B[i])でO(N)でいける.
...
C: 解けなかった. ベクトルの90°回転 A(2, 3) B(6, 6) ABベクトルは(4, 3) Aから反時計回りに90°回転した座標は, (2 - 3, 3 + 4), Bから反時計周りに回転した座標は (6 - 3, 6 + 4)

s8pc-6

B: 解けた. 絶対値の合計の最小化. |x - a1| + |x - a2| + ... + |x - an|の最小値は、a1, a2, ... anの中央値となる.
...
D: 解けなかった. DFSする時に, 1 -> 0にして、最後に0 -> 1. 計算量がわからず.

joi2011yo

D: 解けなかった. dp[0][0] = 1と初期化してしまった.

pakencamp-2019-day3

D: 解けなかった. dp[列][色]で考える. 同じ色同士が隣り合う時、遷移しない.

joi2015ho

B: 解けなかった. 区間DP. メモ化再帰. 円の循環なので、配列を結合させる. ターンは残りのピースの偶奇でわかる.

s8pc

G: bitDP. dpをふたつ用意して、最短距離と、通り数をそれぞれ求める.